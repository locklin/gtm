...
gtm.demo <-function (randomSetup = FALSE, points = 5, samples = 20) {
    ...
    if (randomSetup) {
        ...
        stpResult = gtm.stp1(T, samples, points, 2)
        b = stpResult$beta
        FI = stpResult$FI
        W = stpResult$W
        X = stpResult$X
    ...
    readString = readline()
    for (j in 1:15) {
        trnResult = gtm.trn(T, FI, W, 0, 1, b, quiet = TRUE)
        W = trnResult$W
        b = trnResult$beta
        Y = FI %*% W
    ...
    return(list(X = X, T = T, W = W, Y = Y, beta = b, llh = trnResult$llh, FI = FI))
}

gtm.dist <-function (T, Y, mode = 0) {
    if (mode < 0) {
    ...
    if (yD > 1) {
      ...
      DIST = t(yy) %*% matrix(1, 1, N) + matrix(1, K, 1) %*% tt - 2 * DIST
    } else {
      DIST = matrix(0, K, N)
      for (n in 1:N) {
        for (k in 1:K) {
...
gtm.pmd <-function (T, X, FI, W) {
    D = ncol(T)
    DIST = gtm.dist(T, FI %*% W)
    minDist = apply(DIST, 2, which.min)
    return(matrix(X[minDist, ], ncol = 1))
}

gtm.pmn <-function (T, X, FI, W, b) {
    D = ncol(T)
    DIST = gtm.dist(T, FI %*% W)
...
gtm.ppd1 <-function (t, Y, beta, X) {
    t = matrix(t, nrow = 1)
    D = ncol(t)
    L = ncol(X)
    distResult = gtm.dist(t, Y, 1)
...
gtm.ppd2 <-function (t, Y, beta, X, xDim, yDim) {
    t = matrix(t, nrow = 1)
    N = nrow(t)
    D = ncol(t)
    K = nrow(X)
    L = ncol(X)
    distResult = gtm.dist(matrix(t, nrow = 1), Y, 1)
    R = gtm.resp6(distResult$DIST, distResult$minDist, distResult$maxDist, 
        beta, D, 1)$R
    if (L == 2) {
...
gtm.stp1 <-function (T, noLatVarSmpl, noBasFn, s) {
    ...
    sigma = s * (MU[2] - MU[1])
    FI = gtm.gbf(MU, sigma, X)
    pciResult = gtm.pci.beta(T, X, FI)
    return(list(X = X, MU = MU, FI = FI, W = pciResult$W, beta = pciResult$beta))
}

gtm.stp2 <-function (T, noLatVarSmpl, noBasFn, s) {
    if (s <= 0) {
      stop("Argument s must have strict positive value")
    }
...
gtm.trn <-function (T, FI, W, l, cycles, beta, m = 1, quiet = FALSE, minSing = 0.01) {
    mode = m
    ...
    for (cycle in 1:cycles) {
        ...
        if (attr(cholResult, "rank") != nrow(A)) {
            ...
            W = svdInverse %*% (FI.T %*% (gtmGlobalR %*% T))
          } else {
            oo = order(attr(cholResult, "pivot"))
            W = chol2inv(cholResult)[oo, oo] %*% (FI.T %*% (gtmGlobalR %*% T))
          }
        gtmGlobalDist = gtm.dist(T, FI %*% W, mode)
        if (mode > 0) {
          gtmGlobalMinDist = gtmGlobalDist$minDist
